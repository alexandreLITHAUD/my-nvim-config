return {
	...
	config = function()
		require("catppuccin").setup({
			...
			transparent_background = false, -- sadly doesnt work well at true
		...
		-- Force fix Visual highlight after colorscheme loads
		vim.api.nvim_set_hl(0, "Visual", { bg = "#45475a", fg = "NONE" })
		vim.api.nvim_set_hl(0, "VisualNOS", { bg = "#45475a", fg = "NONE" })

		-- Also fix search highlights that might be causing issues
		vim.api.nvim_set_hl(0, "IncSearch", { bg = "#f9e2af", fg = "#1e1e2e", bold = true })
		vim.api.nvim_set_hl(0, "Search", { bg = "#f9e2af", fg = "#1e1e2e" })
		vim.api.nvim_set_hl(0, "CurSearch", { bg = "#f9e2af", fg = "#1e1e2e", bold = true })
	end,
}
